...
export const member = (app: Elysia) =>
  app.group('member', app => {
    return app
      ...
      .get(
        '/recent',
        async ({ error, payload, query }) => {
          try {
            const response: memberModel.getRecentMember[] =
      ...
      .get(
        ...
        },
        {
          detail: {
            description:
              'Get recent member in organization search by orgId in limit',
            tags: ['Member'],
          },
        },
      )
