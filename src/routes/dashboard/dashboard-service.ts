...
export async function countMembers(organizationId: string): Promise<dashboardModel.memberInformation> {
    ...
    const percentageIncrease = previousMonthCount === 0 ? 100 : ((currentMonthCount - previousMonthCount) / previousMonthCount) * 100;
...
export async function countInvites(organizationId: string): Promise<dashboardModel.inviteInformation> {
