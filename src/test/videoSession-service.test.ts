  ...
  it('should return the video session ID', async () => {
    ...
    dbSelectStub.returns({
      from: sinon.stub().returns({
        where: sinon
          .stub()
          .resolves([{ state: videoSessionStateEnum.UPLOADING }]),
      }),
    })
    dbUpdateStub.returns({
      set: sinon.stub().returns({
        where: sinon.stub().returns({
          returning: sinon.stub().resolves([false]),
        }),
      }),
    })

    const result = await updateVideoSession('videoSessionId', 0, 'userId')
  ...
  it('should remove the video session', async () => {
    dbSelectStub.returns({
      from: sinon.stub().returns({
        where: sinon
          .stub()
          .resolves([{ state: videoSessionStateEnum.CAN_DELETE }]),
      }),
    })
