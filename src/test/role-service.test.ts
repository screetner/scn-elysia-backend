...
describe('getUnassignedRole', () => {
    ...
    it('should return unassigned roles for the given organization ID', async () => {
        ...
        const result: roleModel.roleMemberInformation[] = await getUnassignedRole('org123');
...
describe('getRoleInformation', () => {
    ...
    it('should return role information for the given role ID', async () => {
        ...
        const expectedResponse: roleModel.roleManagement = {
            ...
            rolePermissions: mockRoleInformation.abilityScope as roleModel.rolePermission,
        };
    ...
    it('should throw an error if no role information is found for the given role ID', async () => {
        ...
        expect(getRoleInformation('role123', 'org123')).rejects.toThrow("Role with ID role123 not found in organization org123");
...
describe('assignRole', () => {
    ...
    it('should assign role to users if they do not have any role', async () => {
        ...
        const mockUserRoles = [
            { roleId: 'roleDefault', roleName: roleModel.DEFAULT_ROLE, organizationId: 'org123', username: 'user1' },
            { roleId: 'roleDefault', roleName: roleModel.DEFAULT_ROLE, organizationId: 'org123', username: 'user2' },
        ];
    ...
    it('should throw an error if a user already has a role assigned', async () => {
        const mockNewRole = [{ roleId: 'role123', organizationId: 'org123' }];
        const mockUserRoles = [
            { roleId: 'role456', roleName: 'Admin', organizationId: 'org123', username: 'user1' },
        ];

        setupSelectStub(mockNewRole, mockUserRoles);

        expect(assignRole(['user1'], 'role123', 'org123'))
            .rejects.toThrow("User user1 already has a role assigned");
    ...
    it('should throw an error if a user is not part of the organization', async () => {
        const mockNewRole = [{ roleId: 'role123', organizationId: 'org123' }];
        const mockUserRoles = [
            { roleId: null, roleName: roleModel.DEFAULT_ROLE, organizationId: 'org456', username: 'user1' },
        ];

        setupSelectStub(mockNewRole, mockUserRoles);

        expect(assignRole(['user1'], 'role123', 'org123'))
            .rejects.toThrow("User user1 is not part of the organization");
...
describe('unassignRole', () => {
    ...
    it('should throw an error if default role is not found', async () => {
        ...
        expect(unassignRole('user123', 'org123')).rejects.toThrow("Default role not found in organization org123");
    ...
    it('should throw an error if user is not found or already has the default role', async () => {
        ...
        expect(unassignRole('user123', 'org123')).rejects.toThrow("User user123 not found or already has the default role");
...
describe('changeRoleName', () => {
    ...
    it('should throw an error if the role is not found', async () => {
        ...
        expect(changeRoleName('role123', 'NewRoleName', 'org123')).rejects.toThrow("Role 'NewRoleName' not found in your organization");
    ...
    it('should throw an error if the role name is invalid', async () => {
        ...
        expect(changeRoleName('role123', 'NewRoleName', 'org123')).rejects.toThrow("Cannot change the name of the role 'Default'");
...
describe('deleteRole', () => {
    ...
    it('should throw an error if the role is not found', async () => {
        ...
        expect(deleteRole('role123', 'org123'))
    ...
    it('should throw an error if the role has assigned users', async () => {
        ...
        expect(deleteRole('role123', 'org123'))
            .rejects.toThrow("Cannot delete role with ID role123 because it has assigned users");
...
describe('createRole', () => {
    ...
    beforeEach(() => {
        ...
        insertStub = sinon.stub(db, 'insert').returns({
            values: sinon.stub().returns({
                returning: sinon.stub().resolves([{ roleId: 'newRoleId', roleName: 'New Role' }]),
            }),
        });
    });
    ...
    it('should create a new role with a suffix if roles with the same name exist', async () => {
        selectStub.returns({
            from: sinon.stub().returns({
                where: sinon.stub().resolves([{ roleName: 'New Role' }, { roleName: 'New Role 1' }]),
            }),
        });

        insertStub.returns({
            values: sinon.stub().returns({
                returning: sinon.stub().resolves([{ roleId: 'newRoleId', roleName: 'New Role 2' }]),
            }),
        });
...
describe('updateRolePermission', () => {
    ...
    it('should update the role permission successfully', async () => {
        ...
        const result = await updateRolePermission('role123', roleModel.DEFAULT_PERMISSION, 'org123');
    ...
    it('should throw an error if the role is not found', async () => {
        ...
        expect(updateRolePermission('role123', roleModel.DEFAULT_PERMISSION, 'org123'))
            .rejects.toThrow("Role with ID role123 not found in organization org123");
